...
function autogenerated_cell(cell_id, input) {
    /*
    Creates a new cell after "cell_id" and evaluates the "input" in it.
    If the cell was already created (it contains the "auto_text" below), it is
    resued, i.e. the old contents is removed and replaced by "input" and
    reevaluated.
    */
    var f;
    ...
    f = auto_text + '\n' + input;
    // Make sure that cell_id is an integer (flex passes in a string):
    ...
    if (eval_below == true) {
        ...
        next_cell_id = id_of_cell_delta(cell_id, 1);
        ...
        if (next_cell_input.value.substring(0, auto_text.length) == auto_text) {
            ...
            cell_focus(next_cell_id, false);
            evaluate_cell(next_cell_id, false);
        } else
            insert_new_cell_after_and_evaluate(cell_id, f);
    } else {
        $('#cell_input_' + cell_id).val(f);
        cell_input_resize(cell_id);
        evaluate_cell(cell_id, false);
    }
}
